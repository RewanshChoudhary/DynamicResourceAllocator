...
def test_zomato_adapter_supports_source_filters(tmp_path: Path):
    csv_path = tmp_path / "zomato_filters.csv"
    csv_path.write_text(
        "\n".join(
            [
                "ID,Delivery_person_ID,Delivery_person_Age,Delivery_person_Ratings,Restaurant_latitude,Restaurant_longitude,Delivery_location_latitude,Delivery_location_longitude,Order_Date,Time_Orderd,Time_Order_picked,Weather_conditions,Road_traffic_density,Vehicle_condition,Type_of_order,Type_of_vehicle,multiple_deliveries,Festival,City,Time_taken (min)",
                "0x1,PARTNER1,25,4.5,12.9716,77.5946,12.9816,77.6046,12-02-2022,10:15,10:25,Fog,Jam,1,Meal,motorcycle,1,No,Metropolitian,20",
                "0x2,PARTNER2,30,4.8,12.9816,77.6046,12.9916,77.6146,12-02-2022,11:15,11:25,Sunny,Low,1,Snack,scooter,0,No,Urban,18",
                "0x3,PARTNER3,33,4.1,12.9616,77.5846,12.9716,77.5946,12-02-2022,12:15,12:25,Fog,Jam,1,Drinks,motorcycle,0,Yes,Metropolitian,22",
            ]
        ),
        encoding="utf-8",
    )
    ...
    assert payload["metadata"]["source_filter_match_count"] == 1
    assert payload["metadata"]["source_valid_match_count"] == 1
    assert payload["metadata"]["source_unique_delivery_partners"] == 1
    assert payload["metadata"]["orders_generated"] == 1
    assert payload["orders"][0]["order_id"] == "0x1"
